...
async def varsFunc(client, message):
    ...
    s_c = "No" if not config.SUPPORT_CHANNEL else f"[Channel]({config.SUPPORT_CHANNEL})"
    s_g = "No" if not config.SUPPORT_GROUP else f"[Group]({config.SUPPORT_GROUP})"
    token = "No" if not config.GIT_TOKEN else "Yes"
    sotify = "No" if not (config.SPOTIFY_CLIENT_ID and config.SPOTIFY_CLIENT_SECRET) else "Yes"
    owners = ", ".join(str(ids) for ids in config.OWNER_ID)
