...
from YukkiMusic.utils.database import delete_playlist, get_playlist, get_playlist_names, save_playlist
from YukkiMusic.utils.decorators.language import language, languageCB
from YukkiMusic.utils.inline.playlist import botplaylist_markup, get_playlist_markup, warning_markup
...
async def get_keyboard(_, user_id):
    ...
    for x in _playlist:
        ...
        title = _note["title"].title()
        keyboard.row(InlineKeyboardButton(text=title, callback_data=f"del_playlist {x}"))
    keyboard.row(
...
@app.on_message(filters.command(DELETEPLAYLIST_COMMAND) & filters.private & ~BANNED_USERS)
@language
...
async def play_playlist(client, CallbackQuery, _):
    ...
    try:
        await stream(_, mystic, user_id, result, chat_id, user_name, CallbackQuery.message.chat.id, video, streamtype="playlist")
