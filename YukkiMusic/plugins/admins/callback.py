...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        async def handle_callback_query(command, chat_id, mention):
            if command == "Mute":
                ...
                if int(command) in [1, 3]:
                    if (duration_played - duration_to_skip) <= 10:
                        ...
                        return await CallbackQuery.answer(f"Bot is not able to seek due to total duration has been exceeded.\n\nCurrently played** {bet}** mins out of **{duration}** mins", show_alert=True)
                ...
                try:
                    await Yukki.seek_stream(chat_id, file_path, seconds_to_min(to_seek), duration, playing[0]["streamtype"])
                except:
